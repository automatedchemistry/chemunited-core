...
class EdgeMode(BaseModel, populate_by_name=True):
    ...
    length : float, default=1.0
        Length of the connection in millimeters. Must be >= 0.
    diameter : float, default=1.0
        Diameter of the connection in millimeters. Must be >= 0.
    straight_path : bool, default=False
    ...
    def validate_classification(cls, v):
        if v not in ConnectionType._value2member_map_:
            raise ValueError(
                f"Invalid classification '{v}', must be one of {list(ConnectionType)}"
            )
        return v
