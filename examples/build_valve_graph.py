...
def build_example_graph() -> ExampleGraph:
    ...
    selector_valve = ValveComponentData(
        name="SelectorValve",
        figure="ValveFigure",
        position=(2.0, 0.0),
        angle=0,
...
def _visible_flow_ports(component: ComponentData):
    return [
        ...
        if port.category == ConnectionType.FLOW
    ]
...
def export_pyvis_html(graph: ExampleGraph, output_path: Path = OUTPUT_HTML) -> Path | None:
    try:
        ...
        for (origin, destination), internal_edge in component.internal_edges.items():
            ...
            network.add_edge(
                ...
                title=(
                    ...
                    f"active: {internal_edge.active}<br>"
                    f"length: {internal_edge.length}<br>"
                    f"diameter: {internal_edge.diameter}"
                ),
                color=component_color,
                dashes=not internal_edge.active,
                width=3 if internal_edge.active else 1.5,
                arrows="to",
            )
    ...
    for connection in graph.connections:
        ...
        if (
            connection.origin_port not in _visible_flow_port_numbers(origin_component)
            or connection.destination_port
            not in _visible_flow_port_numbers(destination_component)
        ):
            continue
...
def main():
    ...
    internal_edge_count = sum(
        ...
        and (isinstance(destination, str) or destination in _visible_flow_port_numbers(component))
    )
    print(
